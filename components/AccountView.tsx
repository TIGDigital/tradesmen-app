...
/**
 * Body of the Account tab. Shared between the customer + tradesman tab groups
 * so we don't duplicate avatar-upload + sign-out logic. The settings modal
 * (`/settings`) still exists as a separate entry point; this is the in-tab
 * version with no Cancel/Done — it's a root tab, not a modal.
 */
export function AccountView() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.colors.bg.canvas }}>
      {/* Tab-root header — no back button, just a large title */}
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        <Text style={[t.type.title1, { color: t.colors.text.primary }]}>Account</Text>
      </View>
